...
export default function PackageHero({ PackageData }: any) {
  ...
  return (
    <section className="relative w-full bg-gradient-to-br from-pink-50 via-rose-50 to-white">
    ...
    <nav className="pt-24 pb-4 px-4 sm:px-8 lg:px-20">
        <ol className="flex items-center gap-1 text-sm flex-wrap">
          ...
          <li className="flex items-center gap-1.5">
            <span
              className="px-3 py-1 rounded-full text-xs font-medium bg-pink-50 text-pink-600 border border-pink-100 max-w-[200px] sm:max-w-xs truncate"
      ...
      <div className="max-w-7xl mx-auto px-4  pb-14">
        ...
        {/* Mobile/Tablet: hero image + 4 child images in a single row below */}
        <div className="lg:hidden flex flex-col gap-2">
          <div className="relative aspect-[4/3] sm:h-[320px] rounded-3xl overflow-hidden">
            ...
            <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-semibold text-pink-600 shadow flex items-center gap-1.5">
              <Clock size={13} />
        ...
        <div className="mt-6 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          {/* Title & rating */}
          <div className="p-5 sm:p-7 bg-gradient-to-br from-pink-50/50 to-rose-50/50">
          ...
          {/* Price & CTA */}
          <div className="p-5 sm:p-7 bg-gradient-to-r from-pink-600 to-rose-500 text-white">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">

              <div className="text-center sm:text-left">
                <p className="text-pink-100 text-xs uppercase tracking-wide">Special Offer</p>
                <div className="flex items-center justify-center sm:justify-start gap-2 mt-1">
                  ...
                  {PackageData.discountPrice && (
                    <span className="text-pink-200 line-through text-lg">
                      ₹{PackageData.price}
                    </span>
                  )}
                  <span className="text-pink-100 text-sm">/person</span>
                </div>
                <p className="text-xs text-pink-100 mt-1">Limited time offer • No hidden charges</p>
              </div>

              <button
                onClick={() => setIsOpen(true)}
                className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-white text-pink-600 font-bold shadow-lg hover:scale-105 transition-all duration-300 hover:shadow-xl cursor-pointer"
              >
                Book Now →
              </button>
