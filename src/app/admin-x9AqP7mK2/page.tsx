...
export default function DashboardPage() {
    ...
    const getDashBoardData = async ()=>  {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/admin/dashboard`);
            const data = await response.json();
            setStats({
              hotels: data.data.hotelCount,
              taxis: data.data.taxiCount,
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4 mb-10">
          {[
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-pink-100">{card.value}</h2>
          ...
          <div className="bg-[#1e0d14] rounded-xl p-6 border border-pink-900/40">
            ...
            <div className="space-y-3">
              {[
                {
                  action: "Add Hotel",
                  path: "admin-x9AqP7mK2/hotels/create-hotel",
                },
                {
                  action: "Add Taxi",
                  path: "admin-x9AqP7mK2/taxi/create-taxi",
                },
                {
                  action: "Add Package",
